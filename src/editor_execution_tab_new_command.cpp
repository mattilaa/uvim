...
bool TabNewCommand::execute(Editor& editor, const CommandRequest& request) const
{
    if(request.text.rfind("tabnew", 0) != 0 &&
       request.text.rfind("tabe ", 0) != 0)
        return false;
    return openTabTarget(editor, request.text);
}
} // namespace command::execution
