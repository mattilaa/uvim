...
bool CdRootCommand::execute(Editor& editor, const CommandRequest& request) const
{
    if(request.text != "cdr")
        return false;
    if(editor.projectRoot.empty())
    {
        editor.setStatusMessage("Project root not set");
        return true;
    }
    return setCwd(editor, editor.projectRoot);
}
} // namespace command::execution
