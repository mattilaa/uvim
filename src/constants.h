...
namespace detail
{
    inline bool iequals_suffix(std::string_view str, std::string_view suffix)
    {
        ...
        return std::ranges::equal(str_end, suffix, [](char a, char b) {
...
bool matches_file_patterns(std::string_view filename,
                           ...
                           bool case_sensitive = false)
{
    return std::ranges::any_of(suffixes, [&](std::string_view p) {
        return case_sensitive ? filename.ends_with(p)
                              : detail::iequals_suffix(filename, p);
    });
}
                             ...
                             bool case_sensitive = false)
{
    if(case_sensitive)
    {
        return std::ranges::any_of(patterns, [&](std::string_view pattern) {
...
inline constexpr std::array python_suffixes = {
    std::string_view{".py"}, std::string_view{".pyw"},
    std::string_view{".pyi"}, std::string_view{".pyx"}
};

inline constexpr std::array javascript_suffixes = {
    std::string_view{".js"}, std::string_view{".jsx"},
...
inline constexpr std::array json_suffixes = {
    std::string_view{".json"}, std::string_view{".jsonc"},
    std::string_view{".json5"}
};

inline constexpr std::array yaml_suffixes = {
    std::string_view{".yaml"}, std::string_view{".yml"}
};
