...
bool WriteQuitAllCommand::execute(Editor& editor,
                                  const CommandRequest& request) const
{
    if(request.text == "qw!" || request.text == "wqall!" ||
       request.text == "wqa!")
    {
        saveAllBuffers(editor, true);
        clearAndExit();
    }
    if(request.text != "qw" && request.text != "wqall" &&
       request.text != "wqa" && request.text != "xa")
        return false;
    if(!saveAllBuffers(editor, false))
