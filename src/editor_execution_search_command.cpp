...
bool SearchCommand::execute(Editor& editor, const CommandRequest& request) const
{
    if(request.text.empty() ||
       (request.text.front() != '/' && request.text.front() != '?'))
        return false;
    if(!editor.hasBuffer())
    {
        editor.setStatusMessage("No buffer");
        return true;
    }
    std::string query = request.text.substr(1);
    if(!query.empty())
    {
        editor.addSearchToHistory(query);
        editor.savedCursorX = *editor.cursorX;
        editor.savedCursorY = *editor.cursorY;
        editor.performSearch(query, request.text.front() == '/');
    }
    return true;
}
} // namespace command::execution
